...
@app.get("/")
def root():
    return {"message": "Hello Judge!"}

@app.post("/law_article_prediction")
async def law_article_prediction(request: Request):
    data = await request.json()
    datamodule = TextClassificationData.from_lists(predict_data=[data['data']],batch_size=4)
    predictions = trainer.predict(model_classificaiton, datamodule = datamodule, output="labels")
    return Response(content=str(predictions[0][0]), media_type="application/json")

@app.post("/sentence_prediction")
async def sentence_prediction(request: Request):
    data = await request.json()
    datamodule = TextClassificationData.from_lists(predict_data=[data['data']],batch_size=4)
    predictions = trainer.predict(model_regression, datamodule = datamodule, output="labels")
    predictions = [labels['sentence'][prediction] for prediction_batch in predictions for prediction in prediction_batch]
    return Response(content=str(predictions), media_type="application/json")
